...
const HFheartdatapostsRouter_no_user_id = express.Router({mergeParams: true}); // define dishRouter as an express router

HFheartdatapostsRouter_no_user_id.use( bodyParser.json());
console.log("inside 12  HFheartdatapostsRouter ");

//handle get request to server all dishes
HFheartdatapostsRouter_no_user_id.route('/')  //line 18 in index.js specifies dishRouter is assigned to /dishes
.get((req,res, next) => {
    console.log('HFheartdatapostsRouter_no_user_id.js - get - 17 Query title req.params.user_id => ' , req.params.user_id);
    if( req.params.user_id != undefined ){  //someone is signed in
        const the_user_id = req.params.user_id;
        console.log('HFheartdatapostsRouter_no_user_id.js - get - 19 Query title the_user_id => ' , the_user_id);
        ...
        .then( (data_of_user) => { //dishes is in mongodb as a collection/database
            ...
            console.log("HFheartdatapostsRouter_no_user_id the_user_id : ", the_user_id);
            console.log("HFheartdatapostsRouter_no_user_id 26 data_of_users[0].username : ", data_of_user[0].username);
            ...
            .then( (heartdataposts) => { //dishes is in mongodb as a collection/database
                console.log("HFheartdatapostsRouter_no_user_id.js - 30 get - Todd" + heartdataposts );
                console.log("HFheartdatapostsRouter_no_user_id.js - 31 get - the_user_id -> " + the_user_id );
...
console.log("HFheartdatapostsRouter_no_user_id.js - just before post ");
HFheartdatapostsRouter_no_user_id.route('/:postId')  //line 18 in index.js specifies dishRouter is assigned to /dishes
.post((req,res, next) => {
    //in postman none button selected before send
    console.log('HFheartdatapostsRouter_no_user_id.js - /:postId Query title req.params.postId => ' , req.params.postId);
    console.log('HFheartdatapostsRouter_no_user_id.js - /:postId Query title req.query=> ' , req.query);
    console.log('HFheartdatapostsRouter_no_user_id.js - /:postId Query title req.body=> ' , JSON.stringify( req.body ));
    Heartdataposts.create( req.body)
    .then( (heartdataposts) => { //dishes is in mongodb as a collection/database
        console.log("HFheartdatapostsRouter_no_user_id.js - Todd" + post );
        //res.render('responses', { title: responses });
        res.statusCode = 200;
        //res.setHeader('Content-Type', 'application/json');  // looking for json string
        /* GET Hello World page. */
...
HFheartdatapostsRouter_no_user_id.route('/:commentId/comments') 
.post((req,res,next) => {
    console.log( "HFheartdatapostsRouter_no_user_id - post req.params.commentId -> "+ req.params.commentId);
    Heartdataposts.findById( req.params.commentId)
    .then( (heartdatapost) => {
        if( heartdatapost != null){
            //dish may not exist.   Build it, save it and return it
            console.log( "HFheartdatapostsRouter_no_user_id - post req.body -> "+ req.body );
...
module.exports = HFheartdatapostsRouter_no_user_id;
